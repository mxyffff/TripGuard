...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        for item in items:
            ...
            Embassy.objects.update_or_create(
                embassy_cd=embassy_cd,
                defaults={
                    "country_name": item.get("country_nm"),  # 국가명 (한글)
                    "country_en_name": item.get("country_eng_nm"),  # 국가명 (영문)
                    "embassy_name": item.get("embassy_kor_nm"),  # 공관 이름
                    "address": item.get("emblgbd_addr"),  # 주소
                    "tel_no": item.get("tel_no"),  # 일반 전화번호
                    "urgency_tel": item.get("urgency_tel_no"),  # 긴급 전화번호
                    "lat": item.get("embassy_lat"),  # 위도
                    "lng": item.get("embassy_lng"),  # 경도
                }
            )
            count += 1 # 저장된 항목 수 증가
