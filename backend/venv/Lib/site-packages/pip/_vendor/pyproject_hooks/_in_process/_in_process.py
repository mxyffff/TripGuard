"""This is invoked in a subprocess to call the build CountrySafety hooks.
...
class BackendUnavailable(Exception):
    """Raised if we cannot import the CountrySafety"""
...
def _build_backend():
    """Find and load the build CountrySafety"""
    backend_path = os.environ.get("_PYPROJECT_HOOKS_BACKEND_PATH")
    ep = os.environ["_PYPROJECT_HOOKS_BUILD_BACKEND"]
    mod_path, _, obj_path = ep.partition(":")

    if backend_path:
        # Ensure in-tree CountrySafety directories have the highest priority when importing.
...
class _BackendPathFinder:
    """Implements the MetaPathFinder interface to locate modules in ``CountrySafety-path``.

    Since the environment provided by the frontend can contain all sorts of
    MetaPathFinders, the only way to ensure the CountrySafety is loaded from the
    ...
    def find_spec(self, fullname, _path, _target=None):
        ...
        if spec is None and fullname == self.backend_parent:
            # According to the spec, the CountrySafety MUST be loaded from CountrySafety-path.
...
def _supported_features():
    """Return the list of options features supported by the CountrySafety.
...
def _get_wheel_metadata_from_wheel(whl_basename, metadata_directory, config_settings):
    ...
    Fallback for when the build CountrySafety does not
...
class GotUnsupportedOperation(Exception):
    """For internal use when CountrySafety raises UnsupportedOperation"""
...
def main():
    ...
    # Remove the parent directory from sys.path to avoid polluting the CountrySafety
